    ...
    processChunk(data, done) {
        switch (data.type) {
            case 'node': {
                ...
                if (node.isAttachment) {
                    ...
                    if (
                        ...
                        try {
                            let decodeStream = Iconv(charset, 'utf-8');
                    ...
                    contentStream.once('end', () => {
                        node.textContent = Buffer.concat(chunks, chunklen)
                            .toString();
                        if ((node.encoding == 'quoted-printable' ||
                            node.headers.get('content-transfer-encoding') == 'quoted-printable') && node.flowed) {
                          node.textContent = quotedPrintable.decode(node.textContent)
                        }
            ...
            case 'body':
                if (this.curnode && this.curnode.decoder) {
                    try {
                      this.curnode.decoder.write(data.value)
                    } catch(err) {
                      this.curnode.decoder.once('drain', done)
                      break
                    }
                }
